...
const Issues = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(issuesResponse)) {
          issuesResponse.forEach((issue, index) => {
            console.log(`Issue ${index} structure:`, {
  ...
  const handleAssigneeChange = async (issueId, newAssigneeId) => {
    try {
      const response = await updateIssue(issueId, { assignee: newAssigneeId });
      setIssues(issues.map(issue => 
